...
import { ApiError, communityApi, type CommunityComment, type CommunityPostDetail } from "@/lib/api";
    ...
    <main className="mx-auto max-w-140 px-5 py-12 sm:px-8 sm:py-14">
      ...
      <div className="mt-6">
        <CommunityPostCard
          ...
          signedIn={!!me}
          onChange={load}
          onDeleted={() => router.push("/community/board")}
          linkToDetail={false}
          showExcerpt={false}
        />
        {post.body && (
  ...
  return (
    <div className="mt-8">
      ...
      <div className="mt-3.5 flex flex-col gap-3.5">
        {post.comments.map((c) => (
          <CommentRow key={c.id} comment={c} canModerate={canModerate} signedIn={signedIn} onChanged={onCommented} />
        ))}
      </div>
...
function CommentRow({
  ...
  const [editAnonymous, setEditAnonymous] = useState(comment.is_anonymous);
  const [editAttachments, setEditAttachments] = useState(comment.attachments);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  async function toggleHidden() {
    setBusy(true);
    try {
      if (comment.is_hidden) await communityApi.unhideComment(comment.id);
      else await communityApi.hideComment(comment.id);
      onChanged();
    } finally {
      setBusy(false);
    }
  ...
  async function castVote(value: 1 | -1) {
    setBusy(true);
    try {
      await communityApi.voteComment(comment.id, value);
      onChanged();
    } finally {
      setBusy(false);
    }
  ...
  async function saveEdit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await communityApi.updateComment(comment.id, {
        body: editBody.trim(),
  ...
  if (editing) {
    return (
      <form onSubmit={saveEdit} className="rounded-lg border border-accent-dim bg-surface p-3.5">
        <textarea
          autoFocus
          value={editBody}
          ...
          className="w-full resize-none rounded-md border border-border-strong bg-background px-3.5 py-2.5 text-[14px] leading-[1.5] outline-none focus:border-accent"
        />
        <CommunityAttachmentPicker attachments={editAttachments} onChange={setEditAttachments} max={MAX_COMMENT_ATTACHMENTS} />
        <div className="mt-2 flex items-center justify-between">
          <label className="flex items-center gap-2 text-[13px] text-muted">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="rounded-md border border-border-strong px-3.5 py-2 text-[13px] text-muted hover:border-accent-dim"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={busy}
              className="rounded-md bg-accent px-3.5 py-2 text-[13px] font-semibold text-[#1a2744] hover:opacity-90 disabled:opacity-50"
            >
              {busy ? "Saving…" : "Save"}
            </button>
          </div>
  ...
  return (
    <div className={`rounded-lg border border-border bg-surface p-3.5 ${comment.is_hidden ? "opacity-60" : ""}`}>
      <div className="flex items-center gap-2 font-mono text-[10.5px] text-faint">
        <span className="text-foreground">{comment.author_display}</span>
        <span>{timeAgo(comment.created_at)}</span>
        {comment.is_anonymous && <span className="rounded border border-border-strong px-1.5 py-0.5">Anonymous</span>}
        {comment.edited_at && <span>edited</span>}
        {comment.is_hidden && <span className="rounded border border-danger/40 px-1.5 py-0.5 text-danger">Hidden</span>}
        {comment.is_mine && (
          <button onClick={startEdit} disabled={busy} className="ml-auto hover:text-muted">
            Edit
          </button>
        )}
        {canModerate && (
          <button
            ...
            disabled={busy}
            className={`text-danger hover:underline ${comment.is_mine ? "" : "ml-auto"}`}
          >
            {comment.is_hidden ? "Unhide" : "Hide"}
          </button>
        )}
      </div>
      <p className="mt-1.5 text-[14.5px] leading-[1.5]">{comment.body}</p>
      <CommunityAttachments urls={comment.attachments} compact />
      <div className="mt-2 flex items-center gap-1">
        <button
          ...
          className={`grid h-5.5 w-5.5 place-items-center rounded border text-[10px] disabled:cursor-default ${
            comment.my_vote === 1 ? "border-accent-dim bg-accent/12 text-accent-dim" : "border-border-strong text-muted"
          }`}
        >
          ▲
        </button>
        <span className="min-w-[1.2em] text-center font-mono text-[11px] text-faint">{comment.score}</span>
        <button
          type="button"
          disabled={!signedIn || busy}
          onClick={() => castVote(-1)}
          ...
          className={`grid h-5.5 w-5.5 place-items-center rounded border text-[10px] disabled:cursor-default ${
            comment.my_vote === -1 ? "border-danger/50 bg-danger/10 text-danger" : "border-border-strong text-muted"
          }`}
        >
          ▼
        </button>
