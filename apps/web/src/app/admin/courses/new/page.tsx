...
import { CourseForm, emptyCourseForm, valuesToPayload, type CourseFormValues } from "@/components/CourseForm";
...
export default function NewCoursePage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const created = await adminApi.createCourse(valuesToPayload(values));
      router.push(`/admin/courses/${created.slug}/edit`);
    } catch (err) {
  ...
  return (
    <div className="max-w-140">
      ...
      <form onSubmit={submit} className="mt-6.5">
        <CourseForm values={values} onChange={setValues} slugEditable />
