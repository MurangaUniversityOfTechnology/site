...
function WaitingContent() {
  ...
  const checkStatus = useCallback(async () => {
    if (!id) return;
    try {
      const donation = await donationApi.status(id);
      if (donation.status === "completed") {
        router.push(`/donate/success?id=${id}`);
      } else if (donation.status === "failed" || donation.status === "cancelled") {
        router.push(`/donate/failed?id=${id}`);
      }
    } catch {
        ...
        {!unknown ? (
          <>
            ...
            <p className="mt-3 text-[15px] leading-[1.55] text-[#8f8368]">
              Enter your M-Pesa PIN to approve{amount ? ` KSh ${amount}` : ""}.
            </p>
...
export default function DonateWaitingPage() {
  return (
