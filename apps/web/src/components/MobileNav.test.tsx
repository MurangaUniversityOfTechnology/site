...
describe("MobileNav", () => {
  ...
  test("Profile tab opens an account sheet with Dashboard/Settings when signed in", () => {
    setup("/dashboard", {
      ...
      email: "a@example.com",
      email_verified: true,
      is_admin: false, is_staff: false, is_chairperson: false,
      photo_url: null,
  ...
  test("Profile sheet shows Admin link for admins and calls sign out on click", () => {
    ...
    setup(
      "/dashboard",
      { id: "1", email: "a@example.com", email_verified: true, is_admin: true, is_staff: false, is_chairperson: true, photo_url: null, membership_status: "active", onboarded: true },
      signOut,
    );
    fireEvent.click(screen.getByRole("button", { name: /profile/i }));
  ...
  test("Admin link only shows in the sheet for admins", () => {
    setup("/", { id: "1", email: "a@example.com", email_verified: true, is_admin: false, is_staff: false, is_chairperson: false, photo_url: null, membership_status: "active", onboarded: true });
    fireEvent.click(screen.getByRole("button", { name: /more/i }));
    expect(screen.queryByRole("link", { name: /admin/i })).not.toBeInTheDocument();
  });

  test("Admin link shows in the sheet when me.is_admin is true", () => {
    setup("/", { id: "1", email: "a@example.com", email_verified: true, is_admin: true, is_staff: false, is_chairperson: true, photo_url: null, membership_status: "active", onboarded: true });
    fireEvent.click(screen.getByRole("button", { name: /more/i }));
    expect(screen.getByRole("link", { name: /admin/i })).toHaveAttribute("href", "/admin");
  });
});
