...
import { ApiError, type SignatureImage, type SignatureStatus } from "@/lib/api";

type Mode = "loading" | "preview" | "draw";

type SignatureApi = {
  status: () => Promise<SignatureStatus>;
  image: () => Promise<SignatureImage>;
  save: (imageBase64: string) => Promise<SignatureStatus>;
  remove: () => Promise<void>;
};
...
export default function SignatureCapture({ api, heading, description, previewAlt }: Props) {
  ...
  async function save() {
    ...
    try {
      ...
      const status = await api.save(dataUrl);
  ...
  async function removeSignature() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Couldn't remove the signature.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="max-w-130 rounded-xl border border-border bg-surface p-6">
      <div className="font-mono text-[10.5px] uppercase tracking-[0.18em] text-faint">{heading}</div>
      <p className="mt-2.5 text-[14px] leading-[1.55] text-muted">{description}</p>
      ...
      {mode === "preview" && previewSrc && (
        <div className="mt-5">
          <div className="rounded-lg border border-border-strong bg-white p-3">
            ...
            <img src={previewSrc} alt={previewAlt} className="h-32 w-full object-contain" />
          </div>
          {savedAt && (
